...
function initSquadUI() {
  ...
  // Show squad browser instead of modal
  squadButton.addEventListener('click', () => {
    ...
    // Use the new squad browser component
    if (window.squadBrowser) {
      window.squadBrowser.toggle();
    } else {
      // Fallback to old modal if component not loaded
