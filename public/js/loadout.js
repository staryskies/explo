...
function populateAvailableTowers() {
  ...
  // Get all tower types from towerStats
  const towerTypes = Object.keys(towerStats);
  
  // Create a tower option for each tower type
  towerTypes.forEach(towerType => {
    ...
    towerOption.className = `tower-option ${isUnlocked ? '' : 'locked'}`;
    ...
    // Add lock icon if tower is locked
    if (!isUnlocked) {
      const lockIcon = document.createElement('div');
      lockIcon.className = 'lock-icon';
      lockIcon.textContent = '🔒';
      towerOption.appendChild(lockIcon);
      
      // Add tooltip for how to unlock
      const requiredDifficulty = playerData.lockedTowers[towerType];
      if (requiredDifficulty) {
        towerOption.title = `Complete ${requiredDifficulty} difficulty to unlock`;
      }
    } else {
      // Add click event for unlocked towers
...
function setupEventListeners() {
  ...
  document.querySelectorAll('.selected-slot.empty').forEach(slot => {
    slot.addEventListener('click', () => {
      ...
      const availableTowers = document.querySelectorAll('.tower-option:not(.locked):not(.selected)');
      if (availableTowers.length > 0) {
        alert('Click on an available tower to add it to your loadout');
      } else if (document.querySelectorAll('.tower-option:not(.locked)').length === 0) {
        alert('You need to unlock towers by completing difficulties');
      } else {
