...
class RestCommunicationService {
  ...
  async sendMessage(type, message, squadId) {
    try {
      ...
      if (type === 'global') {
        this.messageCache.global.push(data);
        this.notifyMessageListeners('global', data);
      } else if (type === 'squad' && squadId) {
        if (!this.messageCache.squad[squadId]) {
          this.messageCache.squad[squadId] = [];
        }
        this.messageCache.squad[squadId].push(data);
        this.notifyMessageListeners('squad', data);
      }
